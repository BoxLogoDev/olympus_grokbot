...
- 첫 3개 업무는 `PROVISIONAL` 상태다.
- 시험 기간 동안 평균 품질 점수가 기준 미만이면 `QUARANTINED` 또는 `SUPERSEDED`로 전환한다.
- 부서의 `ACTIVE + PROVISIONAL` 인간 수가 소프트 한도를 넘으면 헤스티아가 생성 이유를 제우스 보고에 포함한다.
...
| 상태 | 의미 |
|---|---|
| `PROPOSED` | 신이 생성안을 제출했으나 아직 등록되지 않음 |
| `PROVISIONAL` | 등록 후 시험 기간 |
| `ACTIVE` | 현재 원자 업무 수행 중 |
| `DORMANT` | 등록되어 있으나 현재 쉬는 중 |
| `QUARANTINED` | 오류·권한 위반·품질 문제로 사용 중지 |
| `SUPERSEDED` | 더 나은 인간이나 새 직무 버전으로 대체 |
| `ARCHIVED` | 장기 미사용이지만 이력으로 보존 |

```mermaid
stateDiagram-v2
    [*] --> PROPOSED
    PROPOSED --> PROVISIONAL: 검증 통과
    PROPOSED --> [*]: 제안 반려
    PROVISIONAL --> ACTIVE: 업무 배정
    ACTIVE --> DORMANT: 업무 완료
    DORMANT --> ACTIVE: 재사용
    PROVISIONAL --> QUARANTINED: 시험 실패
    ACTIVE --> QUARANTINED: 규칙 위반
...
제안이 반려된 것은 인간이 태어난 것이 아니므로 레지스트리 인간 삭제에 해당하지 않는다. 제안 로그는 이벤트 로그에 남는다.
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> READY: 입력·권한·완료조건 충족
    READY --> ASSIGNED: 인간 선택
    ASSIGNED --> RUNNING
    RUNNING --> BLOCKED
...
| `R2` | 공개 게시, main 병합, 운영 배포, 스토어 제출 | 제우스 승인 필요 |
...
각 봇의 실제 설정문은 `prompts/OLYMPUS_Bot_Prompts_v1.0.md`에서 관리한다.
...
OLYMPUS v1.0은 다음 시나리오를 통과해야 한다.
...
- 공개 게시에는 정확한 산출물 해시가 포함된 승인 토큰이 필요하다.
