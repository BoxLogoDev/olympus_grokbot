...
- 운영 계약은 `spec/operations-v1.yaml`, 실제 운영 순서는 `playbooks/agent-operations.md`에 정의한다. 기존 지휘권·단일 직무·89개 슬롯·재시도 상한은 유지한다.
...
- 원형 명세·정지 이미지·짧은 동작의 단일 직무 슬롯 3개와 실행 템플릿을 추가한다. 슬롯은 총 89개이며 실제 인간 등록은 별도다.
...
- v1.0 문서와 프롬프트는 과거 기준선으로 보존한다. v1.1의 채택 조항은 이 문서에 유지하며, 현재 기준선은 v1.3 헌법과 프롬프트다.
...
| `HEPHAESTUS-CHARACTER-ANIMATOR` | 검수된 캐릭터 이미지와 동작 지시로 짧은 애니메이션을 제작한다 | `CHARACTER_MOTION` |
| `HEPHAESTUS-CHARACTER-ILLUSTRATOR` | 승인된 캐릭터 원형과 변형 지시로 정지 이미지를 제작한다 | `CHARACTER_STILL` |
...
| 정지 이미지 | 헤파이스토스 / `HEPHAESTUS-CHARACTER-ILLUSTRATOR` | 승인 원형을 참조한 단일 이미지 |
| 이미지 검수 | 헤라 / `HERA-CONSISTENCY-AUDIT` | 원형 일치 여부·수정 요청 |
| 짧은 동작(선택) | 헤파이스토스 / `HEPHAESTUS-CHARACTER-ANIMATOR` | 검수 통과 이미지를 참조한 단일 동작 |
...
각 봇의 실제 설정문은 `prompts/OLYMPUS_Bot_Prompts_v1.3.md`에서 관리한다.
...
- v1.3 문서를 기준선으로 고정한다. 기존 프로젝트는 정책 변경 영향 검토 후 전환한다.
- YAML 계약을 파싱하고 검증한다.
...
OLYMPUS v1.3 실행 구현은 다음 시나리오를 통과해야 한다. 문서 반영만으로 통과했다고 간주하지 않는다.
