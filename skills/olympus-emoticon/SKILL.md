---
name: olympus-emoticon
description: OLYMPUS 이모티콘 제작의 인계·검수·복구 절차. 헤스티아가 승인한 해당 라인 업무에서 사용한다.
version: 0.1.0
---
...
승인 원형의 표정 6종과 문구·규격 패키지를 검증한다. 목표 플랫폼이 없거나 최신 규격이 없으면 제출 규격 단계는 준비되지 않은 상태로 둔다.
...
- **rules** / `ARTEMIS-PLATFORM-RULE-SCOUT`: 목표 플랫폼의 확인일 있는 규격. 선행: 없음. 필수 점검: PARENT_GOD_REVIEW, OFFICIAL_SOURCE, FORMAT_RULES.
- **copy** / `APOLLO-EMOTICON-COPY`: 표정별 문구. 선행: rules. 필수 점검: PARENT_GOD_REVIEW, COPY_READABILITY, MEANING.
- **expression** / `APHRODITE-EXPRESSION-DESIGNER`: 표정·포즈 제작 지시. 선행: copy. 필수 점검: PARENT_GOD_REVIEW, APPROVED_CANON, EXPRESSION_DIFFERENCE.
- **illustration** / `HEPHAESTUS-CHARACTER-ILLUSTRATOR`: 개별 표정 이미지. 선행: expression. 필수 점검: PARENT_GOD_REVIEW, CANON_CONSISTENCY, ACTUAL_ALPHA.
- **package** / `HEPHAESTUS-IMAGE-PROCESSOR`: 크기·용량·이름을 맞춘 표정 패키지. 선행: illustration, rules. 필수 점검: PARENT_GOD_REVIEW, DIMENSIONS, FILE_SIZE, SET_COMPLETENESS.
