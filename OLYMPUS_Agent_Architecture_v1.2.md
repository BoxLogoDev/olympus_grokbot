...
- v1.0 문서와 프롬프트는 과거 기준선으로 보존한다. v1.1의 채택 조항은 이 문서에 유지하며, 현재 기준선은 v1.2 헌법과 프롬프트다.
...
각 봇의 실제 설정문은 `prompts/OLYMPUS_Bot_Prompts_v1.2.md`에서 관리한다.
...
- v1.2 문서를 기준선으로 고정한다. 기존 프로젝트는 정책 변경 영향 검토 후 전환한다.
- YAML 계약을 파싱하고 검증한다.
...
OLYMPUS v1.2 실행 구현은 다음 시나리오를 통과해야 한다. 문서 반영만으로 통과했다고 간주하지 않는다.
