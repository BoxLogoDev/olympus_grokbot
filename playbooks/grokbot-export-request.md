...
> 작성한 Notion 문서를 다시 읽어 JSON이 비어 있지 않고 실제 파싱되는지 확인하라. 원격 경로만으로 Codex가 파일을 읽을 수 있다고 가정하지 마라.
> 본문이 잘리면 요약본과 전체 자료를 구분하고 수신자가 읽을 수 있는 비공개 전달 방법을 사용하라.
...
수신자의 읽기·파싱·항목 수 대조까지 확인한 뒤 인계 완료로 기록한다. 상세 기준은 [도구 사용 Process](tool-coordination.md)의 내보내기 인계 조건을 따른다. 원본 바이트 해시와 bridge의 정규화 JSON 해시는 구분한다.

Notion 첨부를 사용하면 송신 연결의 업로드 성공과 수신 연결의 다운로드 성공을 따로 확인한다. 수신 측이 첨부를 읽지 못하면 읽을 수 있는 본문/분할 export로 전달하며 원격 경로나 첨부 ID만으로 완료 처리하지 않는다.
