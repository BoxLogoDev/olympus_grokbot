...
구현 버전 0.1.1 / 정책 v1.3 (`30363791228181b986cc94491ab938ee544699f4`).
...
- `review`: 지정한 검수자가 검수 점유 후 정확한 attempt_id·result_digest를 대상으로 판정한다.
  반려는 실패 항목·분류·실패 키·수정 대상·재사용 대상을 포함한다. 제출 파일의 손상·삭제도
  새 반려 근거와 실제 검수 비용으로 기록할 수 있다. 합격은 여전히 모든 파일·입력 검증이 필요하다.
  `repair_tasks`와 그 후속 단계를 함께 무효화한다. 선행 작업이 포함되면 반려된 작업도 그 완료를
  기다린다. 영향 작업에 다른 실행·미확정 검수가 있으면 변경 전체를 거부하므로 먼저 대조·정산한다.
  `reusable_tasks`에는 영향 범위 밖에서 파일·검수 증거가 유효한 DONE 작업만 넣는다.
- `block/reconcile`: 중단 원인 기록 후 원격 결과와 비용을 대조한다. 응답 불명·비용 미확인 상태의
  예약과 점유는 유지한다. 모르는 비용을 0으로 입력하지 않는다. 실제 초과액도 정산하고 신규 배정을 정지한다.
  `RESUME`은 현재 attempt_id(미실행은 null)와 일치해야 한다. 점유 없는 REVIEW 중단은 REVIEW로,
  DRAFT/READY 중단은 의존성을 다시 확인한 대기/준비 상태로 복원한다. 재제작 명령으로 사용하지 않는다.
...
- `update-binding`: 기존 인간·Bot·슬롯을 유지하며 Skill·도구 버전 변경을 증거와 함께 기록한다. 진행 중 점유가 있으면 거부한다. 제작 담당자의 Bot·Skill·도구가 고정 카드와 다르면 새 제작 점유를 거부한다.
- `handoff`: 업무 ID·현재 입력·선행 결과·다음 행동·완료 조건·상위 잔여 한도 및 현재 result/result_digest를 반환한다. 검수자는 이 결과 해시를 review에 사용한다. 다음 행동은 안내이며 claim의 예산·점유·승인 검사를 대신하지 않는다.
...
0.1.1 전환 시 DB schema=1과 기존 이력·비용·점유를 보존한다. `done_when`이 추가된 절차 정의 해시는
이전 것과 다르므로 과거 수동 검증으로 새 버전을 선택할 수 없다. 기존 검증 행을 고치지 말고 새
workflow_ref 버전과 정의를 고정해 별도의 실제 검증을 쌓는다. 신규 운영 후보의 전체 바인딩·검수자
설정을 설치 기록에서 다시 대조한다. 기존 업무 ID를 바꿔 재시도 한도를 초기화하지 않는다.

인계 응답의 next_action은 DRAFT=`wait_dependencies`, READY=`claim_production`, RUNNING=`submit`,
REVIEW=`claim_review`, REVIEWING=`review`, BLOCKED=`reconcile`, DONE=`none`이다.
이전 `production`/`review` 두 값만 처리하던 연결부는 새 상태별 값과 검수 결과 해시를 확인한다.
